...
API + workers for the Staccato pacing scorer. Python 3.12, FastAPI, SQLAlchemy 2
...
# API
uvicorn staccato_backend.main:app --reload
# Worker (needs Postgres; procrastinate schema: `procrastinate schema --apply`)
procrastinate --app=staccato_backend.jobs.app worker
# Cron entry (channel re-scans)
python -m staccato_backend.cron
```

Defaults run on SQLite + local media storage with `STACCATO_AUTH_MODE=dev`
...
STACCATO_TEST_PG_DSN=postgresql+asyncpg://... pytest tests/test_entitlements_concurrency.py
```
...
- **web**: `uvicorn staccato_backend.main:app`
- **worker**: `procrastinate --app=staccato_backend.jobs.app worker`
- **cron**: `python -m staccato_backend.cron` on a schedule
...
(channel fan-outs; throttle via `STACCATO_BATCH_QUEUE_THROTTLE_S`).
...
- Optical (live-capture) scores are recomputed server-side from submitted cut
  times and stay out of canonical pools until promoted in staccato-admin.
