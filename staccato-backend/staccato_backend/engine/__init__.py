...
staccato_backend, so it can be extracted into a standalone OSS repo verbatim.
...
from staccato_backend.engine.scoring import (
    ENGINE_VERSION,
    LABELS,
    build_heatmap,
    label_for_score,
...
def detect_cuts(video_path, threshold: float = 27.0):
    ...
    from staccato_backend.engine.detect import detect_cuts as _detect_cuts
...
def analyze(video_path, threshold: float = 27.0):
    ...
    from staccato_backend.engine.detect import analyze as _analyze
