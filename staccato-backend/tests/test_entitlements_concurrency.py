...
The real row-lock race runs against Postgres (set STACCATO_TEST_PG_DSN, e.g.
postgresql+asyncpg://localhost/staccato_test — CI does this); the SQLite variant
...
from staccato_backend.deps.entitlements import QuotaExhausted, consume_free_analysis
from staccato_backend.models import Base, UsageCounter, User

PG_DSN = os.environ.get("STACCATO_TEST_PG_DSN")
...
@pytest.mark.skipif(not PG_DSN, reason="STACCATO_TEST_PG_DSN not set (Postgres required for row-lock race)")
